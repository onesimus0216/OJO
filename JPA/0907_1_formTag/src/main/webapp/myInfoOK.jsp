...
//	get 방식도 한글이 깨졌었었다. => tomcat setver 7.0부터 get 방식의 한글이 깨지지 않는다.
//	get 방식에서 한글이 깨지면 서버 환경 설정에서 한글이 깨지지 않도록 설정해야 한다.
//	Server 폴더의 server.xml 파일을 열고 Connector 태그에 URLEncoding="UTF-8" 속성을 추가한다.
...
//	out.println(name + "님은 취미는 " + hobbies + " 입니다.<br>");
	
//	checkbox에서 넘어오는 데이터를 받을 때 넘어오는 항목이 여러개일 수 있기 때문에 getParameterValues()
//	메소드로 받아서 배열에 저장해서 처리한다.

	String[] hobbies = request.getParameterValues("hobbies");
	out.println(name + "님은 취미는 " + hobbies + " 입니다.<br>");
	out.println(name + "님은 취미는 " + Arrays.toString(hobbies) + " 입니다.<br>");
	out.println(name + "님은 취미는 ");
	try {
	...
	out.println(name + "님은 " + trip + "에 가고싶어합니다.");
	
	String[] travel = request.getParameterValues("travel");
	out.println(name + "님은 " + travel + "를 가고싶어합니다.<br>");
	out.println(name + "님은 " + Arrays.toString(travel) + "를 가고싶어합니다.<br>");
	out.println(name + "님은 ");
	try {
		for (int i=0; i<travel.length; i++) {
			out.println(travel[i] + " ");
		}
		out.println("를 가고싶어합니다<br>");
	} catch (NullPointerException e) {
		out.println("없습니다.<br>");
	}
