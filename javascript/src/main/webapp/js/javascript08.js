...
//	map(): 배열의 요소들을 대상으로 인수로 지정한 특정 작업(함수)을 실행 결과를 배열로 리턴한다.
...
const text5 = items.map(function (item) { 
	return item.text;
});
console.log(text5);
console.log('19. ==========================');

const text6 = items.map(item => item.text);
console.log(text6);
console.log('20. ==========================');
...
//	findIndex(): 배열에서 인수로 지정된 조건을 만족하는 첫번째 요소의 index를 없으면 -1을 리턴한다. 
...
//	find(): 배열에서 인수로 지정된 조건을 만족하는 첫번째 요소를 없으면 undefined를 리턴한다.
