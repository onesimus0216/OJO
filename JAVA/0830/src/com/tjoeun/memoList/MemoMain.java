...
//	키보드로 수정할 글번호를 입력받아 수정할 글 1건을 얻어와 화면에 표시하고 비밀번호를 입력받아
...
//		삭제할 글번호에 해당되는 글이 존재하면 글을 삭제하고 존재하지 않으면 글이 존재하지
//		않는다는 메시지를 출력한다.
...
//			삭제하기위해 입력한 비밀번호(password)와 삭제할 글의 비밀번호(original.getPassword())를
//			비교한다.
			if (password.equals(original.getPassword())) {
//				비밀번호가 일치하므로 memoList라는 ArrayList에 저장된 메모를 삭제하는 메소드를
//				실행한다.
